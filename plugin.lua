function run(msg)
	...
	.."   *All users list:*\n"
	.."     /users"
	about_txt = "*Channel Manager Robot* v"..bot_version.."\n\n   _- Send Message by SMS_\n   _- Add, Edit & Remove Caption_\n   _- Send Inline Keyboard_\n   _- Remove Forward Info_\n   _- Send Bold,Italic and Cod Text_\n   _- Send Hyper Link_\n   _- Timer for Post to Next Time_\n   _- & More..._\n\n`Made in General_Movies Team and Powered by General_Movies Co. Group`"
	about_key = {{{text = "کانال رسمی" , url = "https://telegram.me/General_Movies"}},{{text = "ربات تیم" , url = "https://telegram.me/General_MoviesBot"}}}
...
`با استفاده از این کلید میتوانید نام کانال، یوزرنیم کانال، آی دی کانال، شماره ارتباطی با سامانه پیامکی، انقضای اکانت، آمار کانال، سازنده ی کانال، تعداد ادمینهای کانال، لیست ادمینهای کانال را در یک لیست ببینید.`]]
	...
	elseif msg.text == "ایدی ادمین " then
		return send_msg(msg.from.id, "`از طریق این ایدی میتوانید با ما ارتباط برقرار کنید.`\n@PvResanGeneral_MoviesBot", true)
	...
	elseif msg.text == "مثال کیبرد اینلاین و مارک داون" then
		inlinekey = {{{text = "کانال تیم" , url = "https://telegram.me/General_Movies"}},{{text = "ربات تیم" , url = "https://telegram.me/General_MoviesBot"}}}
		text = "مثال استایل نویس با فونتهای مختلف:\n*General_Movies Team*\nGeneral_Movies Team\n_General_Movies Team_\n`General_Movies Team`\nجنرال مووی\n`جنرال مووی`\nمثال هایپر لینک:\n[جنرال مووی گروپ](telegram.me/General_Movies)\n[General_Movies Team](telegram.me/General_Movies)\nمثال کیبرد اینلاین:"
	...
	if msg.text:find('/add') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/add 12345678,@General_Movies,9121234567*\n`/add [telegram id],[@channel],[number]`", true)
		end
	elseif msg.text:find('/edit') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/edit 12345678,@General_Movies,9121234567*\n`/edit [telegram id],[@channel],[number]`", true)
		end
	...
	elseif msg.text == "ارسال پست با SMS" then
		users[userid].action = 0
		save_data("users.json", users)
		return send_key(msg.from.id, "`فعلا این بخش غیر فعال است `", keyboard)
	elseif msg.text == "مثال کیبرد اینلاین" then
		return send_inline(msg.from.id, "`تیتر کیبرد اینلاین با قابلیت استفاده از قابلیت مارک داون`", {{{text = "کانال تیم" , url = "https://telegram.me/General_Movies"}}})
	end
